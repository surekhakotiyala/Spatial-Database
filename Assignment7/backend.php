...
### Surekha Kotiyala & Madhuri Komuravelly  
**/
...
function Query3($post){
	...
	foreach($post['sources'] as $source){
		$sql = "
			SELECT ST_AsGeoJSON(wkb_geometry) AS wkb
			FROM {$source}
			WHERE ST_DWithin(ST_SetSRID(wkb_geometry,4269) , ST_MakeEnvelope({$Lon1}, {$Lat1},{$Lon2},{$Lat2},4269),6) 
			AND NOT wkb_geometry IN (SELECT wkb_geometry FROM {$source}
			WHERE ST_SetSRID(wkb_geometry,4269) @ ST_MakeEnvelope({$Lon1}, {$Lat1},{$Lon2},{$Lat2},4269))
			";
		//echo $sql."\n";
